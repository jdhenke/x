...
(define (emit sexpr env)
  (cond ((boolean? sexpr)              (emit-bool sexpr env))
        ...
        ((vector? sexpr)               (emit-vector sexpr env))
...
(define (emit-vector sexpr env)
  (let ((els (vector->list sexpr)))
    (if (null? els)
      (emit-expr "call %Val @make_vec_val(i64 0, %Val* null)")
      (let ((es (map (lambda (s) (emit-quote s env)) els)))
        (define args (emit-expr "call %Args @make_args(i64 " (length es) ")"))
        (enumerate
          (lambda (i e) (emit-line "call void @set_arg(%Args " args ", i64 " i ", %Val " e ")"))
          es)
        (define data (emit-expr "extractvalue %Args " args ", 0"))
        (emit-expr "call %Val @make_vec_val(i64 " (length es) ", %Val* " data ")")))))
...
(define global (list
                 (list
                   ...
                   (list (symbol "list") 1)
                   (list (symbol "list->vector") 49)
                   (list (symbol "list?") 24)
                   (list (symbol "make-vector") 44)
                   ...
                   (list (symbol "vector->list") 50)
                   (list (symbol "vector-length") 47)
                   (list (symbol "vector-ref") 45)
                   (list (symbol "vector-set!") 46)
                   (list (symbol "vector?") 48)
                   )
                  #f))
