...
(define (map-pairs f l)
  (let loop ((l1 l)
    ...
    (cond ((null? (cdr l1)) out)
          ...
          (#t (loop l1 (cdr l2) (cons (f (car l1) (car l2)) out))))))
    ...
    (let ((out (f n last comps)))
      (if out
        ...
        (let inner ((comps comps)
                    ...
                    (rest '()))
          (cond 
            ((null? edges) (error "no more edges"))
            ((null? comps) (loop (cdr edges) (cons (apply append conns) rest) (car edges) (+ n 1)))
...
(define (part-1-stop s)
  (lambda (n e comps)
    (if (< n s)
      #f
      (apply product
        (sublist (sort (map length comps) >) 0 3)))))
