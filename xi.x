  ...
  (let ((env (list (list) env)))
    (define (f k . args)
      (let ((env (list (bind-args args named rest) env)))
        (define (body-loop-cps k body defers last)
          (cond ((null? body)
                 (if (null? defers)
                   (k last)
                   (body-loop-cps (lambda (_) (k last)) defers '() #f)))
                ((and (list? (car body)) (equal? (caar body) 'defer))
                 (body-loop-cps k (cdr body) (cons (cadar body) defers) last))
                (#t (eval (car body) env (lambda (v) (body-loop-cps k (cdr body) defers v))))))
        (body-loop-cps k body '() #f)))
...
(define (bind sexpr env)
  (cond
    ...
    ((equal? (car sexpr) (symbol "defer"))   (bind-pass sexpr env))
