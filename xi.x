  ...
  (let ((thunks (map eval body)))
    (lambda (env k)
      (let ((env (list (make-vector 0 #f) env)))
        (define f
          (cond
            ((and (= num-args 0) (not rest))
              (lambda (k)
                (run-body thunks (list (make-vector frame-size (list #f #f)) env (list 0)) k)))
            ((and (= num-args 1) (not rest))
              (let ((n0 (first named)))
                (lambda (k a0)
                  (let ((v (make-vector frame-size (list #f #f))))
                    (vector-set! v 0 (list n0 a0))
                    (run-body thunks (list v env (list 1)) k)))))
            ((and (= num-args 2) (not rest))
              (let ((n0 (first named)) (n1 (second named)))
                (lambda (k a0 a1)
                  (let ((v (make-vector frame-size (list #f #f))))
                    (vector-set! v 0 (list n0 a0))
                    (vector-set! v 1 (list n1 a1))
                    (run-body thunks (list v env (list 2)) k)))))
            ((and (= num-args 3) (not rest))
              (let ((n0 (first named)) (n1 (second named)) (n2 (third named)))
                (lambda (k a0 a1 a2)
                  (let ((v (make-vector frame-size (list #f #f))))
                    (vector-set! v 0 (list n0 a0))
                    (vector-set! v 1 (list n1 a1))
                    (vector-set! v 2 (list n2 a2))
                    (run-body thunks (list v env (list 3)) k)))))
            (#t
              (lambda (k . args)
                (run-body thunks (list (bind-args args named rest frame-size) env (list num-args)) k)))))
...
(define (eval-call-func sexpr)
  (let ((f-thunk (eval (car sexpr)))
        ...
        (nargs (length (cdr sexpr))))
    (cond
      ((= nargs 0)
        (lambda (env k)
          (f-thunk env (lambda (f) (f k)))))
      ((= nargs 1)
        (let ((a0 (first arg-thunks)))
          (lambda (env k)
            (f-thunk env (lambda (f)
              (a0 env (lambda (v0) (f k v0))))))))
      ((= nargs 2)
        (let ((a0 (first arg-thunks)) (a1 (second arg-thunks)))
          (lambda (env k)
            (f-thunk env (lambda (f)
              (a0 env (lambda (v0)
                (a1 env (lambda (v1) (f k v0 v1))))))))))
      ((= nargs 3)
        (let ((a0 (first arg-thunks)) (a1 (second arg-thunks)) (a2 (third arg-thunks)))
          (lambda (env k)
            (f-thunk env (lambda (f)
              (a0 env (lambda (v0)
                (a1 env (lambda (v1)
                  (a2 env (lambda (v2) (f k v0 v1 v2))))))))))))
      (#t
        (lambda (env k)
          (f-thunk env (lambda (f)
...
(define (eval-lookup c)
  (if (equal? (car c) '_ll)
    (let ((depth (cadr c))
      ...
      (cond
        ((= depth 0)
          (lambda (env k) (k (cadr (vector-ref (car env) offset)))))
        ((= depth 1)
          (lambda (env k) (k (cadr (vector-ref (car (cadr env)) offset)))))
        ((= depth 2)
          (lambda (env k) (k (cadr (vector-ref (car (cadr (cadr env))) offset)))))
        (#t
          (lambda (env k)
            (let loop ((d 0) (env env))
              (if (= d depth)
                (k (cadr (vector-ref (car env) offset)))
                (loop (+ d 1) (cadr env))))))))
    (let ()
      (define (f env k)
